...
select getdate() as timestamp, convert(int,value_in_use) as  trace_enable from sys.configurations where name = ''default trace enabled''
end try
begin catch
select null as timestamp, -100 as trace_enable
end catch',@params=N''
go
exec sp_executesql @stmt=N'begin try
declare @enable int;
