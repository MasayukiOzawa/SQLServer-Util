...
$DestConnectionString = "Server=cdctestserver.mysql.database.azure.com; Port=3306; Database=cdc; Uid=sqladmin@cdctestserver; Pwd=ZAQ!2wsxCDE#; SslMode=Preferred;"

$CaptureInstance = "orders_Capture"
$DestTable = "orders"
...
function New-MySQLConnection() {
    ...
    $con = New-Object MySql.Data.MySqlClient.MySqlConnection -ArgumentList  $ConnectionString
    return $con
}
...
function Get-ColumnValue() {
    ...
    if ( $row[$Column] -eq [System.DBNull]::Value) {
        ...
        switch ($row[$Column].GetType().Name) {
            { @("Int32", "Int64", "Double" , "Decimal", "Byte") -contains $_ } {
...
if ($CaptureCount -gt 0) {
    ...
    $destConnection = New-MySQLConnection -ConnectionString $DestConnectionString
    $destConnection.Open()
