...
OUTER APPLY
(
	-- SQL Server 2014 以前の場合は、In-Memory OLTP の判断は不要なため、本ブロックは SELECT NULL のみとする
	SELECT
		1
	FROM
...
BEGIN 
	PRINT @SchemaName + ':' + @TableName + ':' + @IndexName + ':' +  CAST(@IndexType AS nvarchar(1)) + ':' +  CAST(@PartitionNumber AS nvarchar(3)) + ':' +  CAST(@PartitionCount AS nvarchar(3)) + ':' + CAST(@IncludeColumnstore AS nvarchar(1)) + ':' +  CAST(CAST(@Fragment AS int) AS nvarchar(3)) + ':' +  CAST(@PageCount AS nvarchar(255))
